...
function generateRecommendation(weather, tripType) {
  ...
  if (description.includes("rain") || description.includes("drizzle")) {
    advice += "High chance of rain — carry an umbrella and plan indoor activities. ";
  } else if (description.includes("storm")) {
    advice += "Stormy conditions — prioritize indoor venues and stay safe. ";
  ...
  if (tripType === "Vacation") advice += "Enjoy your vacation and explore local attractions!";
  else if (tripType === "Business") advice += "Allow extra travel time and keep formal attire weather-appropriate.";
  else if (tripType === "Adventure") advice += "Check local trail conditions before heading out.";
...
async function postComment(ticketId, comment) {
  ...
  await axios.post(url, { body: comment, private: false }, {
...
app.post("/recommend", async (req, res) => {
  ...
  if (!ticket_id || !city) return res.status(400).json({ error: "Missing ticket_id or city" });
  try {
    const weatherUrl = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${WEATHER_API_KEY}`;
    const weatherResponse = await axios.get(weatherUrl);
    const recommendation = generateRecommendation(weatherResponse.data, trip_type || "Vacation");
...
app.get("/", (req, res) => res.json({ status: "Weather Travel Middleware is running!" }));
